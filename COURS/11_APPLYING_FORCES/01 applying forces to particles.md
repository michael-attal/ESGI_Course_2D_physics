...
2. On les cumule dans un attribut `sumForces` de la particule.  es forces (`addForce`) à la particule.  
3. Calculer l’accélération : $\vec{a} = \vec{F}_{net} / m$.  
4. Intégrer l’accélération pour obtenir la vitesse.  
5. Intégrer la vitesse pour obtenir la position.  
6. Réinitialiser `sumForces` pour la prochaine frame.  
